...
function* refreshToken(): Generator<any> {
    try {
        ...
        yield put({ type: actionTypes.REFRESH_TOKEN_SUCCESS });
    } catch (err) {
        yield put({ type: actionTypes.LOGOUT_SUCCESS })
    }
}

function* getProfile({ userId }: GetProfileSaga): Generator<any> {
    try {
        const data = yield call(getProfileService, userId);
        yield put({ type: actionTypes.GET_PROFILE_SUCCESS, data });
    } catch (err) {
        yield put({ type: actionTypes.GET_PROFILE_FAILED, err });
    }
}
...
function* sagas() {
    yield all ([
        ...
        takeLatest(actionTypes.GET_PROFILE_REQUEST, getProfile),
    ])
}
